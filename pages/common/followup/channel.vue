...
<script>
	...
	export default {
		...
		methods: {
			textareaAInput(e) {
				this.formBean.result = e.detail.value
			},
			timeChange(e) {
			...
			handleSave() {
				this.formBean.cooperation_id = this.cooperation_id
				this.formBean.next_genjin_at = this.dateTime
				this.formLoading = true
				this.$http('cooperation_followups', this.formBean, 'post').then(r => {
					uni.navigateBack({
						delta: 1
					})
				}).finally(_ => {
			...
			typeChange(e) {
				...
				this.formBean.type = this.types[this.typeInx].text
			},
			desireChange(e) {
