	...
	export default {
		onLoad(opt) {
			...
			this.$http('path').then(r => {
				this.rules = r.data.find(i => i.field == 'CustomerDemand sales').values.filter(i => i.mapping == '公寓')
			})
			this.$http('attribute').then(r => {
				this.allSels = r.data['CustomerDemand sales']
				this.kangxings = this.allSels.kangxing.filter(i => i.mapping == '公寓')
				this.guanzhus = this.allSels.guanzhu.filter(i => i.mapping == '公寓')
				this.intentions = this.allSels.intention.filter(i => i.mapping == '公寓')
				this.transactions = this.allSels.transaction.filter(i => i.mapping == '公寓')
				this.payment_types = this.allSels.payment_type.filter(i => i.mapping == '公寓')
				this.intention_house_types = this.allSels.intention_house_type.filter(i => i.mapping == '公寓')
			})
		},
		data: _ => ({
			rules: [],
